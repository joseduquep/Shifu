...
export default function ProfesoresDashboardPage() {
  ...
  const [selectedDepartamentoIdFiltro, setSelectedDepartamentoIdFiltro] =
    useState("")
  const [selectedMateriaIdFiltro, setSelectedMateriaIdFiltro] = useState("")
  const [selectedSemestreCodigoFiltro, setSelectedSemestreCodigoFiltro] =
    useState("")
  ...
  // eliminado estado no usado: department
  ...
  // eliminado estado no usado: materiasDisponibles
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setEmail(data.email || "")
        // departamento visible solo en lectura (no se guarda en estado)
  ...
  // eliminado efecto no usado para materiasDisponibles
      ...
      try {
        ...
        setMateriasSeleccionadas(
          Array.isArray(json.items)
            ? json.items.map((m: { id: string; nombre: string }) => ({
                id: m.id,
                nombre: m.nombre,
              }))
            : []
  ...
  useEffect(() => {
    const loadMats = async () => {
      try {
        const url = selectedDepartamentoIdFiltro
  ...
  const aplicarFiltros = async () => {
    ...
    try {
      ...
      if (selectedDepartamentoIdFiltro)
        params.set("departamentoId", selectedDepartamentoIdFiltro)
      if (selectedMateriaIdFiltro)
        params.set("materiaId", selectedMateriaIdFiltro)
      if (selectedSemestreCodigoFiltro)
        params.set("semestreCodigo", selectedSemestreCodigoFiltro)
      const res = await fetch(`/api/profesores?${params.toString()}`)
      const json = await res.json()
      setProfesores(
        Array.isArray(json.items) ? (json.items as ProfesorListItem[]) : []
      )
    } catch {
      setProfesores([])
    } finally {
  ...
  const selectedRecord = useMemo(() => {
    return (
      SEMESTERS.find((s) => s.semester === selectedSemester) ?? SEMESTERS[0]
    )
  }, [selectedSemester])
      ...
      <section className="min-h-[calc(100dvh-4rem)] px-6 py-10">
        <div className="mx-auto max-w-6xl">
          <header className="flex items-center justify-between">
            <h1 className="text-3xl md:text-4xl font-medium">Panel Docente</h1>
          </header>

          <nav className="mt-8">
            <div className="inline-flex rounded-lg border border-white/15 p-1 bg-white/5">
              <button
                type="button"
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-5 gap-3">
                  <div className="md:col-span-2">
                    <select
                      ...
                      onChange={(e) => {
                        setSelectedDepartamentoIdFiltro(e.target.value)
                        setSelectedMateriaIdFiltro("")
                      }}
                  ...
                  <div className="md:col-span-2">
                    <select
                      ...
                      onChange={(e) =>
                        setSelectedMateriaIdFiltro(e.target.value)
                      }
                      className="w-full h-11 rounded-xl bg-[#0b0d12] text-white/90 border border-white/15 px-3 focus:outline-none focus:ring-2 focus:ring-primary/50"
                    >
                  ...
                  <div>
                    <select
                      ...
                      onChange={(e) =>
                        setSelectedSemestreCodigoFiltro(e.target.value)
                      }
                      className="w-full h-11 rounded-xl bg-[#0b0d12] text-white/90 border border-white/15 px-3 focus:outline-none focus:ring-2 focus:ring-primary/50"
                    >
                ...
                <form className="mt-6 space-y-4" onSubmit={handleSaveProfile}>
                  ...
                  <div className="pt-2">
                    <button
                      type="submit"
                      disabled={
                        isSaving || (isNewProfile && !selectedDepartamentoId)
                      }
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start gap-6">
                  ...
                  <div className="md:flex-1 w-full">
                    ...
                    <div className="mt-2 min-h-[140px] rounded-xl border border-white/15 bg-[#0b0d12] p-4 text-white/85">
    ...
    const run = async () => {
      try {
        const res = await fetch(
          `/api/materias/suggest?q=${encodeURIComponent(q)}`,
          { signal: ctrl.signal }
        )
        const data = await res.json()
  ...
  return (
    <div className="rounded-xl border border-white/15 bg-[#0b0d12] p-2">
      <div className="flex flex-wrap gap-2">
        {values.map((tag, idx) => (
          <MateriaChip
            key={(tag.id ?? tag.nombre) + idx}
            nombre={tag.nombre}
            onRemove={() => remove(tag.id, tag.nombre)}
          />
        ))}
        <input
          value={input}
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        <span className="text-xs text-white/50">
          La materia queda registrada para sugerencias y filtros.
        </span>
      </div>
        ...
        <div className="mt-2 max-h-48 overflow-auto rounded-lg border border-white/10 bg-[#121621]">
          {suggestions.map((s) => (
            <button
              ...
              onMouseDown={() => addById(s.id, s.nombre)}
            >
              {s.nombre}
            </button>
          ))}
