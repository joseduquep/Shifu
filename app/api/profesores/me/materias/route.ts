...
const BodySchema = z.object({
  materiaIds: z.array(z.string().uuid()).min(0),
...
export async function GET() {
  ...
  let prof: { id?: string; departamento_id?: string } | null = null
  try {
  ...
  if (!prof) {
    const profProfile = await supa
      .from("profiles")
      .select("full_name")
      .eq("id", user.id)
      .maybeSingle()
    const fullName = (profProfile.data as { full_name?: string } | null)
      ?.full_name
    if (fullName) {
  ...
  const items = ((data as Row[] | null) || [])
    .filter((r) => r.activo)
    .map((r) => ({ id: r.materia_id, nombre: r.materias?.nombre || "" }))
...
export async function PUT(req: NextRequest) {
  ...
  for (const mid of materiaIds) {
    const { error: upErr } = await supabaseAdmin
      .from("profesores_materias")
      .upsert(
        { profesor_id: prof.id, materia_id: mid, activo: true },
        { onConflict: "profesor_id,materia_id" }
      )
    if (upErr)
