...
export async function GET() {
  ...
  try {
    const res = await supabaseAdmin
      .from('profesores')
      .select('id, nombre_completo, email, bio, departamento_id, departamentos:departamento_id ( id, nombre, universidades:universidad_id ( id, nombre ) )')
      .eq('user_id', user.id)
      .maybeSingle()
    data = res.data
    error = res.error
...
export async function PUT(req: NextRequest) {
  ...
  if (profErr && String(profErr.message).toLowerCase().includes('does not exist')) {
    const byEmail = await supabaseAdmin
      .from('profesores')
      .select('id')
      .eq('email', user.email)
  ...
  if (!prof) {
    ...
    if (!('departamentoId' in parsed.data) || !parsed.data.departamentoId) {
      return NextResponse.json({ error: 'Seleccione un departamento para crear su perfil' }, { status: 400 })
    }
    ...
    if (!dep) return NextResponse.json({ error: 'Departamento no existe' }, { status: 400 })
    ...
    if (parsed.data.nombreCompleto) {
      ...
      if (!byName.error && byName.data) {
        ...
        if (!parsed.data.departamentoId || byName.data.departamento_id === parsed.data.departamentoId) {
          matched = byName.data
        }
      }
    }
    ...
    if (matched) {
      ...
      if (parsed.data.departamentoId) updateExisting.departamento_id = parsed.data.departamentoId
      ...
      const tryUserId = await supabaseAdmin
        ...
        .select('id')
        .limit(1)
      const canUseUserId = !(tryUserId.error && String(tryUserId.error.message).toLowerCase().includes('user_id'))
      if (canUseUserId) insertPayload.user_id = user.id
      const { error: insErr } = await supabaseAdmin.from('profesores').insert(insertPayload)
  ...
  const b = parsed.data
  if (b.nombreCompleto !== undefined) payload['nombre_completo'] = b.nombreCompleto
  if (b.email !== undefined) payload['email'] = b.email
  if (b.bio !== undefined) payload['bio'] = b.bio
  if (b.departamentoId !== undefined) payload['departamento_id'] = b.departamentoId
