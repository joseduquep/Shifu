...
describe('Flujo de Búsqueda de Profesores', () => {
	...
	test('debe realizar búsqueda por nombre de profesor', async () => {
		...
		const response = await fetch(`/api/profesores?q=${encodeURIComponent(searchQuery)}`)
		const data = await response.json()

		expect(global.fetch).toHaveBeenCalledWith(`/api/profesores?q=${encodeURIComponent(searchQuery)}`)
	...
	test('debe filtrar profesores por departamento', async () => {
		...
		const response = await fetch(`/api/profesores?departamentoId=${departamentoId}`)
		const data = await response.json()

		expect(global.fetch).toHaveBeenCalledWith(`/api/profesores?departamentoId=${departamentoId}`)
		expect(data.items).toHaveLength(1)
	...
	test('debe filtrar profesores por universidad', async () => {
		...
		const response = await fetch(`/api/profesores?universidadId=${universidadId}`)
		const data = await response.json()

		expect(global.fetch).toHaveBeenCalledWith(`/api/profesores?universidadId=${universidadId}`)
		expect(data.items).toHaveLength(1)
	...
	test('debe combinar múltiples filtros en búsqueda', async () => {
		...
		expect(global.fetch).toHaveBeenCalledWith(`/api/profesores?${params.toString()}`)
		expect(data.items).toHaveLength(1)
	})
