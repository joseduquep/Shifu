// File: `lib/admin/dao/profesores.ts`
...
export async function listProfesores() {
    const { data, error } = await supabaseAdmin
        .from('profesores')
        .select('id, user_id, nombre_completo, email, bio, fotografia, departamento_id, created_at')
...
export async function createProfesor(input: ProfesorCreate) {
    ...
    const { data: prof, error: e2 } = await supabaseAdmin
        ...
        .insert({
            ...
            user_id: userId,
            fotografia: input.fotografia ?? null, // NUEVO
        })
        .select('*')
        .single()
    if (e2) throw e2
...
export async function updateProfesor(input: ProfesorUpdate) {
    const { data, error } = await supabaseAdmin
        ...
        .update({
            ...
            bio: input.bio,
            departamento_id: input.departamento_id,
            fotografia: input.fotografia, // NUEVO
        })
        .eq('id', input.id)
